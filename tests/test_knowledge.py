...
class KnowledgeTests(unittest.TestCase):
    def setUp(self):
        ...
        self.repo.mkdir()
        for name in ("bin", "docs", "memory", "tests", "assets"):
        ...
        for name in ("README.md", "AGENTS.md", ".gitignore", ".project-starter.json",
                     "LICENSE.project-starter", "example.jeeves_prompt"):
...
''')
    ...
    def test_application_validation_only_runs_after_successful_full_checks(self):
        self.tool("shellcheck", "import sys\nsys.exit(0)\n")
        suite = self.repo / "tests/test_knowledge.py"
        suite.write_text("import unittest\nclass Probe(unittest.TestCase):\n    def test_probe(self): pass\n")
        self.tool("bundle", "import os, sys\nfrom pathlib import Path\n"
                  "Path(os.environ['EVENTS']).write_text(' '.join(sys.argv[1:]))\n"
                  "sys.exit(int(os.environ.get('APP_EXIT', '0')))\n")
        self.run_command("bin/check")
        self.run_command("bin/check", "--documents-only")
        self.assertFalse(self.events.exists())
        self.run_command("bin/check", "--full")
        self.assertEqual(self.events.read_text(), "exec rake validate")
        result = self.run_command("bin/check", "--full", extra={"APP_EXIT": "17"}, check=False)
